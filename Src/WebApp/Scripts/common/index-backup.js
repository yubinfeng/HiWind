...
(function ($) {
    ...
    window.mainpage.loadMenu = function (id) {
        ...
        if ($.array.contains(["SysMenu099", "SysMenu100"], id)) {
            $.get(id == 'SysMenu099' ? "/Scripts/common/nav-api-menu-data.json" : "/Scripts/common/nav-doc-menu-data.json", function (menus) {
                root.children = menus;
            ...
            // root.children = [];
            // t.tree("loadData", [root]);
            $.get("/Main/GetChildrenMenus/?menuparentid=" + id + "&id=rood=" + Math.floor(Math.random() * 1000), function (menus) {
                root.children = menus;
                t.tree("loadData", [root]);
            }, "json");
        }
    ...
    window.mainpage.loadNavMenus = function (callback) {
        ...
        $.get("/Main/GetTopMenu/?id=rod=" + Math.floor(Math.random() * 1000), function (menus) {
    ...
    window.mainpage.loadFavoMenus = function () {
        ...
        $(favoMenuTree).tree("load", "/Scripts/common/favo-menu-data.json");
    };
    ...
    window.mainpage.mainTabs.loadHash = function (hash) {
        ...
        $.get("/Scripts/common/nav-api-menu-data.json", function (menus) {
    ...
    window.donate.reload = function () {
        ...
        $.ajax({
            url: "/Scripts/common/donate-data.json", type: "get", dataType: "json",
    ...
    window.link.reload = function () {
        ...
        $.ajax({
            url: "/Scripts/common/link-data.json", type: "get", dataType: "json",
