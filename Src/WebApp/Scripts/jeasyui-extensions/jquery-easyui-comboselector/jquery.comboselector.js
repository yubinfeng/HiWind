...
(function ($, undefined) {
    ...
    function create(target) {
        ...
        if (opts.lazyLoad) {
            ...
            $[opts.method](opts.url, $.extend({}, opts.queryParams || {}, { page: opts.pageNumber || $.fn.datagrid.defaults.pageNumber, rows: opts.rows || $.fn.datagrid.defaults.pageSize }), function (data) {
                //初始加载 判断是否为对象 ybf 2014-5-29 
                data = $.util.isPlainObject(data) ? data : $.string.toObject(data);
                //end
                state.data = data ? ($.util.likeArrayNotString(data) ? data : data.rows) : [];
                initValue();
            });
        ...
        function initValue() {
            if (state.data && state.data.length) {
                ...
                //初始加载空值 ybf 2014-5-29 
                //if (!opts.nullable && $.array.isNullOrEmpty(opts.value)) {
                 //   opts.value = [null];
                //}
                //end
    ...
    $.fn.comboselector.methods = {
        ...
        //getSelectData: function (jq) { return jq.each(function () { $(this).comboselector("panel").parent().parent().find(".grid-selector").datagrid("getSelections"); }); } //ybf2014-11-22
